function [phantom] = MRSI_MNI_phantom_1mm(B0, slice, mets, T2_met, T2_lip, MemoryOptions)
arguments
    B0 (1,1) double
    ...
    wm = spm_vol([filePath, '/..', '/MNI152_1mm/mni_icbm152_wm_tal_nlin_sym_09a.nii']);
    gm = spm_vol([filePath, '/..', '/MNI152_1mm/mni_icbm152_gm_tal_nlin_sym_09a.nii']);
    pd = spm_vol([filePath, '/..', '/MNI152_1mm/mni_icbm152_pd_tal_nlin_sym_09_brain.nii']);
    skull = spm_vol([filePath, '/..', '/MNI152_1mm/mni_icbm152_pd_tal_nlin_sym_09_brain_skull.nii']);
    ...
    for y = 1:size(phantom.wm,1)
        for x = 1:size(phantom.wm,2)
            ...
            for m = 1:length(metabolites)
                ...
                if(strcmp(name, 'H2O'))
                    if(phantom.pd(y,x,slice) > 1)
                        conc = phantom.pd(y,x,slice);
                    else
                        conc = 0;
                    end
                elseif(strcmp(name, 'Lipids'))
                    conc = phantom.skull(y,x,slice)*1000;
                else
