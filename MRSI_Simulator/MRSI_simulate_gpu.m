...
function [out, voxel_sig] = MRSI_simulate_gpu(traj, phantom, gMax, B0, T2star)
...
phan_x = [phantom.x];
phan_x = gpuArray(reshape(phan_x, [size(phantom, 1), size(phantom, 2)]));

%create an matrix of y coordinates (used for speedup)
phan_y = [phantom.y];
phan_y = gpuArray(reshape(phan_y, [size(phantom, 1), size(phantom, 2)]));
...
for excite=1:size(gradient,1)
    ...
    for k=1:readout_length
        ...
        phantom_sig = gpuArray(zeros(size(new_phantom)));
            ...
            for y = 1:y_phantom_size
                for m = 1:length(new_phantom(x,y).met)
                    ...
                    if k~=1
                        ...
                        phantom_sig(x,y) = phantom_sig(x,y) + val*trace(trace_matrix * new_phantom(x,y).d{1});
        ...
        if k == 1
            ...
            for x = 1:x_phantom_size
                for y = 1:y_phantom_size
                    for m = 1:length(new_phantom(x,y).met)
                        trace_matrix = new_phantom(x,y).met(m).Fx + 1i*new_phantom(x,y).met(m).Fy;
                        phantom_sig(x,y) = phantom_sig(x,y) + trace(trace_matrix * new_phantom(x,y).d{m});
                        %voxel_sig(excite, k, x, y) = phantom_sig(x,y);
                    end
                end
            end
        ...
        S_gpu(excite, k) = sum(phantom_sig, 'all');
    end
