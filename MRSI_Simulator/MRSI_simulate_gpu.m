...
function [out, voxel_sig] = MRSI_simulate_gpu(traj, phantom, gMax, B0, MemoryOptions)
arguments
    traj (1,1) Trajectory
    phantom (:, :) struct
    gMax (1,1) double = 30
...
for m = 1:length(phantom.met)
    ...
    shifts = gpuArray(single(phantom.met(m).shifts));
    Fz = gpuArray(single(phantom.met(m).Fz));
    HAB = gpuArray(single(phantom.met(m).HAB));
    ...
    for excite=1:size(gradient,1)
        ...
        for k=1:readout_length
            ...
            if(~strcmp(traj_name, 'cartesian') || k <= 2)
                [H, H_inv, is_diag] = calculate_H(shifts, Fz, HAB, grad_matrix, gradientTime(excite,k), B0, idx);
            end
...
%grad matrix
function [H, H_inv, is_diag] = calculate_H(shifts, Fz, HAB, gradients, time, B0, idx)
...
%gyromagnetic ratio
gamma=gpuArray(42577000);  %[Hz/T]
gradients = gradients(:);
%compute the ppm_eff from above
ppm_eff = (gradients.*shifts(1)/1e6 + gradients - B0)*1e6/B0;
%remove empty voxels
ppm_eff = ppm_eff(idx);
ppm_diff = ppm_eff - shifts(1);

%convert to rads/s
shift_rads = ppm_diff*gamma*B0*(2*pi/1e6);
shift_rads = reshape(shift_rads, 1,length(shift_rads));

diag_idx = get_diag_index(size(Fz,1),1);
Fz = Fz(diag_idx);
...
new_HAB = repmat(HAB, [1,1,sum(idx)]);
...
if(isdiag(new_HAB(:,:,1)))
    ...
    H = pagefun(@expm, HAB_evolve);

    %for i = size(H, 3):-1:1
    %    H(:,:, i) = expm(HAB_evolve(:,:,i));
    %end
    %H_inv = conj(H);
    is_diag = false;
end
